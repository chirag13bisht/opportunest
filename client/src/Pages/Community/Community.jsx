...
const Community = () => {
  ...
  const updateLikeorUnlike = async (postId, userId, action) => {
    try {
      const url = action === 'like' 
        ...
        : 'https://opportunest-1.vercel.app/api/auth/unlike';
  ...
  const postCommunity = async (e) => {
    ...
    try {
      const res = await axios.post('https://opportunest-1.vercel.app/api/auth/post', {
        email,
        sharedtext,
      });
              ...
              user.posts?.map((post, postIndex) => (
                <div key={`${index}-${postIndex}`} className="post-content">
                 <Link className="post-link" to={`/profile/${user.basicdata[0].firstname}-${user.basicdata[0].lastname}/${user._id}`} > <div className="user-info">
                  <img src={user && user.image ?`https://opportunest-1.vercel.app/Images/${user.image}` : avatar} alt="profile_photo" className="profile-photo" />
